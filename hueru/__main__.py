...
async def get_bridge():
    ...
    if CONFIG_FILE.exists() and CONFIG_FILE.stat().st_size > 0:
        try:
            ...
            print(f"Warning: Could not parse config file at {CONFIG_FILE}. Starting fresh.")
            config = {}
    ...
    async with aiohttp.ClientSession() as session:
        if host:
            # Try to connect with existing host
            try:
                ...
                print(f"Could not connect to bridge at {host}. Trying to re-discover.")
                host = None
                bridge = None
        ...
        if not app_key:
            print("No application key found. Please press the link button on the bridge.")
            for i in range(MAX_RETRIES):
                try:
                    ...
                    print(f"Button not pressed. Retrying in {RETRY_DELAY_SECONDS}s... ({i+1}/{MAX_RETRIES})")
        ...
        try:
            ...
            # If we fail here, the app_key might be bad.
            # We don't want to delete the whole config, just the key to trigger re-creation.
            if "app_key" in config:
                del config["app_key"]
                CONFIG_FILE.write_text(json.dumps(config, indent=2))
                print("Removed potentially invalid application key. Please try again.", file=sys.stderr)
            sys.exit(1)
        finally:
            if bridge:
                await bridge.close()
